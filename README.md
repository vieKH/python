...
1.1. Имя форкнутого репозитория *должно* быть `python-L<номер лабы>` (потенциально допускается форкнуть репозиторий один раз и выполнять лабораторные в различных его ветках, но поскольку курс меняется на ходу, это не самый удобный вариант);  
